...
library(enrichplot) # plot
library(ggplot2) # plot
library(topGO) # GO
library(Rgraphviz) # topGO 可視化
library(pheatmap)
```
...
df <- read.csv("raw_count_data.csv", header  = T)
...
## 結果を書き出し
write.csv(as.data.frame(resOrdered_B_vs_A), "DEG_res_B_vs_A.csv")
write.csv(as.data.frame(resOrdered_C_vs_A), "DEG_res_C_vs_A.csv")
write.csv(as.data.frame(resOrdered_C_vs_B), "DEG_res_C_vs_B.csv")
...
## output
output_filename <-  "normalized_counts.csv"
write.csv(normalized_counts, output_filename)
...
## output
output_filename <- "rlog_transformed.csv"
write.csv(normalized_rld, output_filename)
...
# pdfに保存
ggsave("heatmap_output.pdf", plot = p, width = 10, height = 8)
```
...
# pdfに保存
ggsave("ego_B_vs_A_up_barplot.pdf", plot = p, width = 10, height = 8)
