...
# ホームディレクトリにある .Rprofile のパスを作成（Windows, macOS, Linux 共通）
rprofile_path <- file.path(Sys.getenv("HOME"), ".Rprofile")

# .Rprofile が存在しなければ作成する（存在する場合は上書きする場合に注意）
if (!file.exists(rprofile_path)) {
  file.create(rprofile_path)
  cat(".Rprofile ファイルを作成しました：", rprofile_path, "\n")
} else {
  cat(".Rprofile ファイルは既に存在します：", rprofile_path, "\n")
}

# .Rprofile に書き込む内容（OSに応じたユーザーライブラリパスの自動設定）
rprofile_content <- c(
  "# --- 自動でユーザー専用ライブラリパスを設定するコード ---",
  "if (Sys.info()[[\"sysname\"]] == \"Windows\") {",
  "  userLib <- file.path(Sys.getenv(\"USERPROFILE\"), \"Documents\", \"R\", \"win-library\",",
  "                       paste0(R.version$major, \".\", R.version$minor))",
  "} else if (Sys.info()[[\"sysname\"]] == \"Darwin\") {  # macOS",
  "  userLib <- file.path(Sys.getenv(\"HOME\"), \"Library\", \"R\",",
  "                       paste0(R.version$major, \".\", R.version$minor), \"library\")",
  "} else {  # Linux 等",
...
# .Rprofile に内容を書き込む（上書きするので、既存の設定がある場合は注意）
writeLines(rprofile_content, con = rprofile_path)
cat("設定内容を .Rprofile に書き込みました。\n")
...
# もし途中で「GenomeInfoDbDataが必要です」と表示された場合は、次を実行
